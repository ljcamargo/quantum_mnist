...
def apply_inverse_fft_2d(fft_data: np.ndarray) -> np.ndarray:
    ...
    return np.real(np.fft.ifftshift(fft_data))
...
def process_image(image: np.ndarray, output_dir: str, image_idx: int, mask_factor: float = 0.3):
    ...
    # Step 7: Visualize reconstruction comparison with FFT analysis
    print(f"Processing {image_name} - Step 7: Full Pipeline Comparison with FFT Analysis")
    fig, axes = plt.subplots(2, 4, figsize=(20, 10))
    
    # Row 1: Original images and reconstructions (same as before)
    ...
    diff1 = np.abs(image - reconstructed)
    axes[0, 2].imshow(diff1, cmap='hot', vmin=0, vmax=np.max(diff1))
    axes[0, 2].set_title('Direct Diff')
    axes[0, 2].axis('off')
    
    axes[0, 3].imshow(final_reconstructed, cmap='gray', vmin=0, vmax=1)
    axes[0, 3].set_title('JSON Reconstruction\n(JSON → FFT → iFFT)')
    axes[0, 3].axis('off')
    
    # Row 2: Add the two requested FFT comparisons
    axes[1, 0].imshow(image, cmap='gray', vmin=0, vmax=1)
    axes[1, 0].set_title('Original Image')
    axes[1, 0].axis('off')
    
    diff2 = np.abs(image - final_reconstructed)
    axes[1, 1].imshow(diff2, cmap='hot', vmin=0, vmax=np.max(diff2))
    axes[1, 1].set_title('JSON Diff')
    axes[1, 1].axis('off')
    
    # NEW: Direct reconstruction FFT (magnitude)
    masked_fft_mag = np.log(np.abs(masked_fft) + 1e-8)
    axes[1, 2].imshow(masked_fft_mag, cmap='gray')
    axes[1, 2].set_title('Direct FFT\n(Log Magnitude)')
    axes[1, 2].axis('off')
    
    # NEW: JSON reconstruction FFT (magnitude)
    json_fft_mag = np.log(np.abs(reconstructed_fft_from_json) + 1e-8)
    axes[1, 3].imshow(json_fft_mag, cmap='gray')
    axes[1, 3].set_title('JSON FFT\n(Log Magnitude)')
    axes[1, 3].axis('off')
